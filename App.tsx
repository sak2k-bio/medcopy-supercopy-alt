...
export default function App() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setInputs(prev => {
        ...
        // Logic to make Batch Mode, Carousel Mode, Summarizer Mode, and Image Mode mutually exclusive
  ...
  const removeImage = () => {
    setInputs(prev => ({ ...prev, image: undefined }));
  };
  ...
  const handleGenerate = async () => {
    ...
    // Check persona for Image/Summarizer modes if needed, usually they need a persona too
    if ((inputs.summarizerMode || inputs.imageMode) && !inputs.persona.trim()) {
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
          ...
          <div className="lg:col-span-5 flex flex-col h-full">
            <div className={`bg-white dark:bg-slate-900 rounded-xl shadow-sm border p-6 flex flex-col gap-6 relative transition-colors ${inputs.summarizerMode ? 'border-orange-200 dark:border-orange-900/40' : (inputs.imageMode ? 'border-blue-200 dark:border-blue-900/40' : 'border-slate-200 dark:border-slate-800')}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <select
                      ...
                      value={inputs.format}
                      onChange={handleInputChange}
                      disabled={inputs.batchMode || inputs.carouselMode || inputs.summarizerMode || inputs.imageMode} 
                      className={`w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none text-slate-900 dark:text-slate-100 ${(inputs.batchMode || inputs.carouselMode || inputs.summarizerMode || inputs.imageMode) ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      ...
                      {(!inputs.batchMode && !inputs.carouselMode && !inputs.summarizerMode && !inputs.imageMode) && <option className="font-bold text-indigo-700 dark:text-indigo-400">Multi-Format Exploder</option>}
                    </select>
                 </div>
                 <div>
              ...
              <div>
                <div className="flex justify-between items-end mb-1">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    ...
                    {inputs.summarizerMode 
                        ...
                        : (inputs.batchMode ? 'Keywords / Core Theme' : (inputs.carouselMode ? 'Carousel Topic' : (inputs.imageMode ? 'Analysis Focus / Context' : 'Topic / Raw Thought')))}
                  </label>
                  ...
                  <div className="flex flex-wrap gap-4">
                      ...
                      {/* Vision/Image Mode Toggle */}
                      <label className="flex items-center gap-2 cursor-pointer group select-none">
                        <input 
                          type="checkbox"
                          name="imageMode"
                          checked={inputs.imageMode}
                          onChange={handleInputChange}
                          className="w-4 h-4 text-blue-600 border-slate-300 dark:border-slate-600 rounded focus:ring-blue-500 dark:bg-slate-800 cursor-pointer"
                        />
                        <span className={`text-xs font-medium transition-colors flex items-center gap-1 ${inputs.imageMode ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-blue-500 dark:group-hover:text-blue-400'}`}>
                          <Eye size={12} />
                          Vision
                        </span>
                      </label>
                ...
                <textarea
                  ...
                  placeholder={
                    inputs.summarizerMode 
                      ...
                      : (inputs.imageMode ? "Describe what to focus on (e.g., 'Check for rash characteristics' or 'Analyze layout')." : (inputs.batchMode ? "Enter keywords. We'll generate random angles." : "What is this content about?"))
                  }
                  className={`w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all resize-none placeholder:text-slate-400 dark:placeholder:text-slate-600 text-slate-900 dark:text-slate-100 ${inputs.batchMode ? 'h-16 ring-1 ring-purple-100 dark:ring-purple-900' : (inputs.carouselMode ? 'h-24 ring-1 ring-pink-100 dark:ring-pink-900' : (inputs.summarizerMode ? 'h-16 ring-1 ring-orange-100 dark:ring-orange-900' : (inputs.imageMode ? 'h-16 ring-1 ring-blue-100 dark:ring-blue-900' : 'h-24')))}`}
                />

                {inputs.enableDistillation && !inputs.batchMode && !inputs.carouselMode && !inputs.summarizerMode && !inputs.imageMode && (
                ...
                {inputs.batchMode && (
                   <div className="flex items-center gap-3 mt-2 bg-purple-50 dark:bg-purple-900/20 p-2 rounded-lg border border-purple-100 dark:border-purple-900/30">
                      <span className="text-xs font-bold text-purple-700 dark:text-purple-300 whitespace-nowrap">Batch Count: {inputs.batchCount}</span>
                      <input 
                        type="range" 
                        min="2" 
                        max="10" 
                        ...
                        onChange={handleInputChange}
                        className="w-full h-1.5 bg-purple-200 rounded-lg appearance-none cursor-pointer dark:bg-purple-800 accent-purple-600"
                      />
                   </div>
                )}

                {inputs.imageMode && (
                  <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1.5 flex items-center gap-1 font-medium">
                    <Eye size={10} />
                    Vision Mode: Upload an image to generate persona-driven captions and analysis.
                  </p>
                )}
                {/* ... existing mode messages ... */}
                {inputs.summarizerMode && (
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className={`block text-sm font-medium flex items-center gap-1.5 ${inputs.summarizerMode ? 'text-orange-700 dark:text-orange-400 font-bold' : (inputs.imageMode ? 'text-blue-700 dark:text-blue-400 font-bold' : 'text-slate-700 dark:text-slate-300')}`}>
                    {inputs.imageMode ? <ImageIcon size={14} /> : <BookOpen size={14} />}
                    {inputs.summarizerMode 
                        ? 'Source Text to Summarize (Required)' 
                        : (inputs.imageMode ? 'Upload Image for Analysis (Required)' : 'Medical Context (RAG / Reference)')}
                  </label>
                  {!inputs.imageMode && (
                ...
                {inputs.imageMode ? (
                  <div className="w-full">
                     {!inputs.image ? (
                        <div className="border-2 border-dashed border-blue-200 dark:border-blue-900/40 rounded-lg bg-blue-50/50 dark:bg-blue-900/10 h-64 flex flex-col items-center justify-center relative hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors cursor-pointer">
                            <input 
                                type="file" 
                                accept="image/*" 
                            ...
                            <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">Click to Upload Image</p>
                            <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">Supports JPG, PNG (Max 10MB)</p>
                        </div>
                     ) : (
                        <div className="relative h-64 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-800 group bg-black/5 dark:bg-black/40 flex items-center justify-center">
                            <img src={inputs.image} alt="Uploaded preview" className="max-h-full max-w-full object-contain" />
                            <div className="absolute top-2 right-2 flex gap-2">
                                <button 
                                    onClick={removeImage}
                                    className="bg-red-500 text-white p-1.5 rounded-full hover:bg-red-600 shadow-md transition-transform hover:scale-105"
                                    title="Remove image"
                                >
                                    <X size={14} />
                                </button>
                ...
                {!inputs.summarizerMode && !inputs.imageMode && (
              ...
              <div className="mt-auto pt-2 flex gap-3">
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-lg text-white font-semibold shadow-md transition-all ${
                    isGenerating || (inputs.summarizerMode ? !inputs.context : (inputs.imageMode ? !inputs.image : (!inputs.persona || !inputs.topic)))
                    ...
                    <>
                      ...
                      {inputs.batchMode 
                        ...
                        : (inputs.carouselMode 
                            ...
                            : (inputs.summarizerMode
                                ? 'Summarize Text'
                                : (inputs.imageMode ? 'Analyze Image' : (inputs.format === 'Multi-Format Exploder' ? 'Explode Content' : 'Generate Draft'))
                              )
                          )
                      }
                    </>
              ...
              <div className="border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                <div className="flex items-center gap-3">
                   <div className="flex items-center gap-2">
                      <div className={`h-2 w-2 rounded-full ${generatedResult ? (generatedResult.batchOutput ? 'bg-purple-500' : (generatedResult.carouselOutput ? 'bg-pink-500' : (inputs.imageMode ? 'bg-blue-500' : 'bg-teal-500'))) : 'bg-slate-300 dark:bg-slate-600'} ${isGenerating ? 'animate-pulse' : ''}`}></div>
                        ...
                        {generatedResult?.multiFormatOutput 
                          ...
                          : (generatedResult?.batchOutput 
                              ...
                              : (generatedResult?.carouselOutput 
                                  ? 'Carousel Output' 
                                  : (inputs.summarizerMode && generatedResult ? 'Summary Result' : (inputs.imageMode && generatedResult ? 'Vision Analysis' : 'Output Preview'))))}
                      </h3>
                   </div>
                ...
                {generatedResult && !generatedResult.batchOutput && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`text-sm flex items-center gap-1.5 px-3 py-1.5 rounded-md transition-all border ${
                        sheetSaveSuccess
